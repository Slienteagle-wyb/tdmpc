...
class KeypointMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        ...
        if self.time_independent:
            ...
            visible_dims_mask = torch.cat([torch.ones((B, T, self.action_dim), dtype=torch.bool, device=device),
                                           visible_dims, torch.ones((B, T, self.context_dim),
            ...
            visible_dims = torch.repeat_interleave(visible_kps, repeats=self.keypoint_dim, dim=-1)
            visible_dims_mask = torch.cat([torch.ones((B, self.action_dim), dtype=torch.bool, device=device),
                                           visible_dims, torch.ones((B, self.context_dim), dtype=torch.bool, device=device), ], dim=-1)
...
def test():
    ...
    self = LowdimMaskGenerator(4, 0, max_n_obs_steps=3, action_visible=True)
    mask = self(shape=(2, 16, 4))
