...
class DSSac(nn.Module):
    ...
    def pi(self, z):
        ...
        action, log_prob, mean = self._pi.get_action(z)
        return action

    def pi_log_prob(self, z):
        """Samples an action from the learned policy (pi)."""
        action, log_prob, mean = self._pi.get_action(z)
...
class MoSacSimDssm:
    ...
    def _optim_initialize(self, total_epochs):
        ...
        self.pi_optim = create_optimizer(model=self.model._pi, optim_id=self.cfg.optim_id,
                                         lr=self.cfg.pi_lr)
        # parameters for automatic entropy tuning
        self.target_entropy = -torch.prod(torch.Tensor(self.cfg.action_dim).to(self.device)).item()
        self.log_temp = torch.zeros(1, requires_grad=True, device=self.device)
        self.temp = torch.exp(self.log_temp).item()
        self.temp_optim = torch.optim.AdamW([self.log_temp], lr=self.cfg.temp_lr)
    ...
    def plan(self, obs, hidden, eval_mode=False, step=None, t0=True):
        ...
        # choose one of the best action trajectory
    ...
    def update_pi(self, zs):
        ...
        for t, z in enumerate(zs):
            action_pi, log_pi = self.model.pi_log_prob(z)
            Q = torch.min(*self.model.Q(z, action_pi))
            pi_loss += (self.temp * log_pi - Q).mean()
        ...
        self.temp_optim.zero_grad(set_to_none=True)
        with torch.no_grad():
            _, log_pi = self.model.pi_log_prob(zs[0])
        temp_loss = (-self.log_temp * (log_pi.detach() + self.target_entropy)).mean()
        temp_loss.backward()
    ...
    def _td_target(self, next_z, reward):
        ...
        action_pi, log_pi = self.model.pi_log_prob(next_z)
        q = torch.min(*self.model_target.Q(next_z, action_pi)) - self.temp * log_pi
    ...
    def update(self, replay_buffer, step):
        ...
        pi_loss, temp_loss = self.update_pi(zs)
