...
class DSSMRE(nn.Module):
    """Task-Oriented Latent Dynamics (TOLD) model used in TD-MPC."""

    def __init__(self, cfg):
        super().__init__()
        extend_dim = cfg.env.num_vis_gates * 4
        self.cfg = cfg
        self._dynamics = DGruDyna(cfg)
        self._reward = h.mlp(cfg.hidden_dim, cfg.mlp_dim, 1)
        self._pi = h.mlp(cfg.latent_dim, cfg.mlp_dim, cfg.action_dim)
        self._Q1 = nn.Sequential(nn.Linear(cfg.latent_dim + cfg.action_dim, cfg.mlp_dim),
                                 nn.LayerNorm(cfg.mlp_dim), nn.Tanh(), nn.Linear(cfg.mlp_dim, cfg.mlp_dim),
                                 nn.LayerNorm(cfg.mlp_dim), nn.ELU(), nn.Linear(cfg.mlp_dim, 1))
        self._Q2 = nn.Sequential(nn.Linear(cfg.latent_dim + cfg.action_dim, cfg.mlp_dim),
                                 nn.LayerNorm(cfg.mlp_dim), nn.Tanh(), nn.Linear(cfg.mlp_dim, cfg.mlp_dim),
                                 nn.LayerNorm(cfg.mlp_dim), nn.ELU(), nn.Linear(cfg.mlp_dim, 1))
        ...
        self.state_projector = nn.Sequential(nn.Linear(in_features=cfg.env.num_vis_gates * 4,
...
class TdMpcSimDssmRE:
    ...
    @torch.no_grad()
    def estimate_value(self, z, actions, hidden, obs_gates_plan):
        ...
        z_fuse = self.model.state_projector(obs_gates_plan) + z
        G += discount * torch.min(*self.model.Q(z_fuse, self.model.pi(z_fuse, self.cfg.min_std)))
    ...
    def plan(self, obs, hidden, eval_mode=False, step=None, t0=True, fine_tuning=False):
        ...
        obs, obs_gates = torch.split(obs, [19, 8], dim=-1)
        ...
        if num_pi_trajs > 0:
            ...
            obs_gates_pi = obs_gates.repeat(num_pi_trajs, 1)
            for t in range(self.plan_horizon):
                z_fuse_pi = self.model.state_projector(obs_gates_pi) + z_pi
                pi_actions[t] = self.model.pi(z_fuse_pi, self.cfg.min_std)
        ...
        obs_gates_plan = obs_gates.repeat(self.cfg.num_samples + num_pi_trajs, 1)
        ...
        for i in range(self.cfg.iterations):
            ...
            value, reward_mean = self.estimate_value(z_plan, actions, hidden_plan, obs_gates_plan)
    ...
    def update(self, replay_buffer, step):
        ...
        obs, obs_gate = torch.split(obs, [19, 8], dim=-1)
        next_obses, next_obses_gate = torch.split(next_obses, [19, 8], dim=-1)
        ...
        z_fuse = self.model.state_projector(obs_gate) + z
        ...
        zs = [z_fuse.detach()]  # obs embedding for policy learning
        ...
        for t in range(self.cfg.horizon):
            ...
            Q1, Q2 = self.model.Q(z_fuse, action[t])
            z, hidden, reward_pred = self.model.next(z, action[t], hidden)
            z_fuse = self.model.state_projector(next_obses_gate[t]) + z
            with torch.no_grad():
                z_fuse_online = self.model.state_projector(next_obses_gate[t]) + online_next_zs[t]
                td_target = self._td_target(z_fuse_online, reward[t])
